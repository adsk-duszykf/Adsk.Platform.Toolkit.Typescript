...
const serviceAccount =
	await client?.helper.createSecureServiceAccount(serviceAccountInfo);
...
const authToken = await client?.helper.getServiceAccountThreeLeggedToken(
	"your_client_id",
	serviceAccount?.account.serviceAccountId,
...
const accountDetails = await client?.api.serviceAccounts
	.byServiceAccountId(serviceAccount?.account.serviceAccountId)
	.get();

// Output:
// Service Account ID: 2a3b4c5d-6e7f-8g9h-0i1j-2k3l4m5n6o7p, status: ENABLED
console.log(
	`Service Account ID: ${accountDetails?.serviceAccountId}, status: ${accountDetails?.status}`,
);
