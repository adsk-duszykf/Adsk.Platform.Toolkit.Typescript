...
import type { TokenPostRequestBody } from "./generatedCode/authentication/v2/token/index.js";
import type { BaseAuthenticationClient } from "./generatedCode/baseAuthenticationClient.js";
import {
	type AuthToken,
	GranttypeObject,
} from "./generatedCode/models/index.js";
import {
...
export class AuthenticationClientHelper {
	...
	createPKCEAuthenticationUrl(
		...
		return createPKCEAuthenticationUrl(
			clientId,
			redirectUri,
			scope,
			codeChallenge,
	...
	createAuthenticationUrl(
		...
		return createAuthenticationUrl(
			clientId,
			redirectUri,
			scope,
			nonce,
	...
	createTwoLeggedAutoRefreshToken(
		...
		return async () => {
			...
			const isExpired =
				!currentToken || currentToken.expiresAt.getTime() - Date.now() < 10000;

			if (isExpired) {
				const newToken = await this.generateTwoLeggedToken(
					clientId,
					clientSecret,
					scopes,
				);
	...
	async generateServiceAccountThreeLeggedToken(
		clientId: string,
		clientSecret: string,
		jwt: string,
		scope: string[],
