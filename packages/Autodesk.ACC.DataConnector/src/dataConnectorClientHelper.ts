...
import { BaseDataConnectorClient } from "./generatedCode/baseDataConnectorClient.js";
import { DataRequest } from './generatedCode/models/index.js';
...
export class DataConnectorClientHelper {
    ...
    async *getAllRequests(accountId: string): AsyncIterableIterator<DataRequest> {
        ...
        const getRequests = async (offset: number, pageSize:number):Promise<DataRequest[]>=>{
            const page = await this.api.accounts.byAccountId(accountId).requests.get( {queryParameters:{offset, limit:pageSize}});
            return page?.results??[];
        }
