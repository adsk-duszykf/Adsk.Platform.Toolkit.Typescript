...
import type { AuthToken } from "@adsk-platform/authentication/models";
import type { HttpClient } from "@microsoft/kiota-http-fetchlibrary";
import * as jose from "jose";
import type { BaseSecureServiceAccountClient } from "./generatedCode/baseSecureServiceAccountClient.js";
import type { Key, ServiceAccountInfo } from "./models/index.js";
...
export class SecureServiceAccountClientHelper {
	...
	async createSecureServiceAccount(serviceAccount: ServiceAccountInfo) {
		...
		const key = await this.api.serviceAccounts
			.byServiceAccountId(account.serviceAccountId)
			.keys.post();
	...
	async getServiceAccountThreeLeggedToken(
		clientId: string,
		serviceAccountId: string,
		scope: string[],
		key: Key,
	): Promise<AuthToken> {
		const jwt = await this.generateAssertionJWT(
			clientId,
			serviceAccountId,
			scope,
			key.privateKey,
		...
		const authToken =
			await this.authClient.helper.generateServiceAccountThreeLeggedToken(
				clientId,
				serviceAccountId,
				jwt,
				scope,
