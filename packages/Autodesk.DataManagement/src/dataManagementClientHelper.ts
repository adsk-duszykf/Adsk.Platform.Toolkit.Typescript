import type { BaseAutodeskDataManagement } from "./GeneratedCode/DataMgt/baseAutodeskDataManagement";
import type { V1RequestBuilder } from "./GeneratedCode/DataMgt/data/v1";
import type { BaseAutodeskOSSClient } from "./GeneratedCode/OSS/baseAutodeskOSSClient";
import type { V2RequestBuilder } from "./GeneratedCode/OSS/oss/v2";
import type {
	FileVersionNode,
	FolderNode,
	HubNode,
...
export class DataManagementClientHelper {
	...
	constructor(
		private dataMgtClient: BaseAutodeskDataManagement,
		ossClient: BaseAutodeskOSSClient,
	) {
	...
	async getHubs(): Promise<HubNode[]> {
		const hubs = await this.dataMgtClient.project.v1.hubs.get({
			queryParameters: { filterextensionType: ["hubs:autodesk.bim360:Account"] },
		});
		return (
			hubs?.data
				?.map<HubNode>((x) => ({
	...
	async getProjects(hubId: string): Promise<ProjectNode[]> {
		const projects = await this.dataMgtClient.project.v1.hubs
			.byHub_id(hubId)
			.projects.get();
		return (
			projects?.data
	...
	async getTopFolders(hubId: string, projectId: string): Promise<FolderNode[]> {
		...
		const folders = await this.dataMgtClient.project.v1.hubs
			.byHub_id(hubId)
			.projects.byProject_id(projectId)
			.topFolders.get();
	...
	async getSubFolders(
		...
		const folders = await this.dataMgtClient.data.v1.projects
			.byProject_id(projectId)
			.folders.byFolder_id(fromFolderId)
	 ...
	 * @memberof DataManagementClientHelper
	 */
	async getLatestFileVersions(
		projectId: string,
		fromFolderId: string,
	): Promise<FileVersionNode[]> {
	...
	fixProjectId(projectId: string) {
		return projectId.toLowerCase().startsWith("b.")
			? projectId
			: `b.${projectId}`;
	}
}
